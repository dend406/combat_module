...
# Завантаження попередньо навченої моделі Haarcascades для детекції силуету людини
fullbody_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + 'haarcascade_upperbody.xml')
...
while True:
    ...
    # Детекція силуету людини
    bodies = fullbody_cascade.detectMultiScale(gray, scaleFactor=1.2, minNeighbors=3, minSize=(40, 40))
    ...
    cv2.imshow('Bodies Tracking', frame)
